<%@ Page Language="C#" AutoEventWireup="true" CodeFile="surveyAnswerView.aspx.cs" Inherits="Webmag_Employe_surveyManage_surveyAnswerView" %>
...
<body>
    <form action="surveyAnswerDelete.aspx?pid=<%=survey.ID%>" method="post" id="pageControlForm" name="pageControlForm" onsubmit="return checkDeleteSubmit('pageControlForm');" >
    <table  class="resultTable" width="800" cellspacing="0" cellpadding="0" style="border-collapse: collapse;">
        <caption style="text-align: left">
            &nbsp;&nbsp;&nbsp;&nbsp;调查问卷结果统计：<%=survey.Name%>&nbsp;&nbsp; 
        </caption>
        <thead>
          ...
          %>
                 <tr onMouseOver="this.style.backgroundColor='#f3f3f3';return true;"
                  ...
                  <td><%=surveyQuestionLists[i].AnswerA%>
                        <br />
                        <%=surveyAnswerLists[i].AnswerANum %>
                  </td>
                  <td><%=surveyQuestionLists[i].AnswerB%>                        <br />
                        <%=surveyAnswerLists[i].AnswerBNum %></td>
                  <td><%=surveyQuestionLists[i].AnswerC%>                        <br />
                        <%=surveyAnswerLists[i].AnswerCNum %></td>
                  <td><%=surveyQuestionLists[i].AnswerD%>                        <br />
                        <%=surveyAnswerLists[i].AnswerDNum %></td>
