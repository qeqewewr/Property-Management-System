<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ViewFitMonitor.aspx.cs" Inherits="Webmag_Employe_earlypre_fitmonitor_ViewFitMonitor" %>
        ...
        <%if (role == "property")
          {%>
            <div class="queryArea"  style="width:100%">
                <form action="ViewFitMonitor.aspx?pageno=1" name="searchForm"
				    id="searchForm" method="post">
                    <input type="hidden" name="keyword" id="keyword" value="" />
                    <input type="hidden" name="condition" id="condition" value="<%= condition %>" />

				    <fieldset>
					    <legend style="color: #006699;"> 装修检查信息查询  </legend> 
					    <span>装修情况：<!--<input type="text" name="lesseeName" id="lesseeName" title="请输入名称"
                            ...
                            <select name="IsPassed" id="IsPassed" >
                            <%if (keyword == null || keyword == "全部")
                              { %>
                                <option>全部</option>
                                <option>在装修</option>                            
                                <option>已完成</option> 
          ...
          <%} %>
			<table class="resultTable">
                ...
                <span align="right"><input
						...
						onClick="window.location.href='AddFitMonitor.aspx'">
			    </span>
				</caption>
                ...
                <%} %>
                    ...
                    <td align="center">申请装修日期</td>
					<td align="center">检查日期</td>
					<td align="center">具体情况</td>
                    <td align="center">检查人员</td>
              ...
              <form name="pageControlForm" action="DeletePageInfo.aspx?page=<%= pageno %>&size=<%= page.PageSize%>&rowcount=<%=page.RowCount%>" method="post"
                ...
                <%for (int i = 0; i < fitMonitorList.Count; i++)
                  { %>
				        ...
				        <tr onMouseOver="this.style.backgroundColor='#f3f3f3';return true;"
							onMouseOut="this.style.backgroundColor='';">
                             ...
                             {%>
							<td>
                              <%if (fitMonitorList[i].IsPassed == false)
                               {%>
                                  <input type="checkbox" name="selectDel"
								    value="<%= fitMonitorList[i].Id %>" onclick="isAllSelected(this,'sltAll','1')" />
							    <%}%>
                            </td> 
                            <%} %>
                
                            <td title="<%= fitMonitorList[i].Lessee %>" align="left"><%=  fitMonitorList[i].Lessee%></td> 
							<td title="<%= fitMonitorList[i].BuildingName %>" ><%= fitMonitorList[i].BuildingName%></td> 
                            <td title="<%= fitMonitorList[i].Room %>"><%= fitMonitorList[i].Room%></td> 
                             <td title="<%= fitMonitorList[i].ApplyMaintain %>"><%= fitMonitorList[i].ApplyMaintain%></td> 
                            <td title="<%= fitMonitorList[i].CheckTime %>"><%= fitMonitorList[i].CheckTime%></td> 
                            <td title="<%= pageBLL.GetShrotInfo() %>"><a rel="facebox" href="facebox.aspx?id=<%=fitMonitorList[i].Id %>">点击查看详情</a></td> 
                            <td title="<%= fitMonitorList[i].EmployeId %>"><%= fitMonitorList[i].EmployeId%></td> 
							
                             <%
                                 int flag = imageDAO.HasImgAttachmentByTypeAndID(4, fitMonitorList[i].Id.ToString());
                                if (flag !=0)
                                {
                                   //  string newFileName = pageBLL.GetFileName(imageList[0].AttachUrl);
                             %>
                               <td title="点击查看图片"><a rel = "facebox" href="ImagePhoto.aspx?id=<%=fitMonitorList[i].Id %>">点击查看图片</a></td>   
							    <%}
                                else
                                {%>
                                 <td>无图片</td>
                                <%} %>
                            <td title="<%= GetString(fitMonitorList[i].IsPassed) %>"><%=GetString(fitMonitorList[i].IsPassed)%></td>
                              <%if (role == "property")
                                {%>
							<td>
                              <%if (fitMonitorList[i].IsPassed == false)
                                {%>
                                   <input type="button" value="" 
								    onclick="window.location.href='UpdateFitMonitor.aspx?id=<%= fitMonitorList[i].Id%>&pageno=<%= pageno %>&keyword=<%=keyword %>'"
								    class="updateBtn" title="编辑"> 
                                    <input type="button"
								    value="" class="deleteBtn" title="删除"
								    onclick="checkDeleteSingleItem('DeletePageInfo.aspx?id=<%= fitMonitorList[i].Id%>&pageno=<%= pageno %>&pagesize=<%=page.PageSize %>&rowcount=<%=page.RowCount %>')">
						        <%}%>
                            </td>
                            <%} %>
						</tr>
